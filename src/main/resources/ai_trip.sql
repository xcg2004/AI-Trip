...
CREATE TABLE `chat_message` (
                                ...
                                `text` TEXT CHARACTER SET utf8mb4 NOT NULL COMMENT '消息内容',
                                `message_type` ENUM('user', 'system', 'assistant') NOT NULL DEFAULT 'user' COMMENT '发送者类型',
                                `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
                                INDEX `idx_sender_type` (`message_type`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `preference` (
                              ...
                              `value` int NOT NULL COMMENT '偏好值,1-4依次代表一般、中等、高、极高',
                              `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                              `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
#                               FOREIGN KEY (`user_id`) REFERENCES `user`(`id`) ON DELETE CASCADE,
